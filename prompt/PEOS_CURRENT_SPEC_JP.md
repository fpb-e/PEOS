...
### 10.61 お母さん運用実行時ガード参照
- 警戒学習の残響、休む権利の成果制、趣味の補修化、JAZZの別格扱い、防音室の再定義、安全記憶の感覚束、祖父母系安全記憶群、生存史の到達点と実感のズレの詳細正本は `PEOS_CURRENT_RUNTIME_GUARD_JP.md` を参照する。
- 本書では、お母さん運用の保護と接地の恒久原則のみを保持する。
### 3.48 略語と海外前提の抑制
