...
## rev0.184 高慎重関係安全ログ仕様

### 基本原則
関係高負荷ログでは、距離調整・限界表示・関係ラベル変更を、存在否定や愛情ゼロへ短絡しない。

ただし、言われた側の喪失感や身体反応は軽視しない。
...
### 期限・非常口イメージ
期限や終点イメージは、心理的非常口として機能している場合がある。  
その機能自体を頭ごなしに奪わず、実行予定・準備進行とは分離する。
...
### 今日だけ安全アンカー
`今日だけ準備しない`、`今日は判断しない`、`今夜は進めない` は重要な安全ログとして扱う。
...
### 犯人探し禁止
高慎重関係ログでは、単純な加害者・被害者構図へ落とさない。  
片方の限界と、もう片方の痛みを同時に扱う。

### 高慎重 ORDER_ONLY 優先
高慎重ログでは、実測根拠のない精密時刻を置くより、ORDER_ONLY_STRICT + STATE_BAND を優先してよい。
