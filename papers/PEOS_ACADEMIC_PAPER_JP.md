...
## rev0.300 Academic Addendum: Self-Reported Conformance vs Recomputed Invariants
...
## rev0.301 Academic Addendum: Stable Evidence Names, Event-Typed Timestamps, and Channel-Typed Artifacts

A revision number is useful provenance but poor logical identity for recurring evidence categories. When every release emits a new revision-bearing evidence filename, conceptual indexing becomes coupled to release count. rev0.301 introduces stable semantic evidence names and stores revision identity inside the manifest and artifact metadata. Historical names remain immutable compatibility records.

Temporal provenance also requires entity typing. The timestamp of a post visible in a screenshot, the arrival time of the user turn containing that screenshot, and the completion time of the resulting artifact are distinct events. Availability of one timestamp cannot fill another event's field.

Finally, artifact generation and artifact display are separated. A session log is a file payload; the conversational channel should normally carry only a receipt. This reduces duplication and accidental content leakage while preserving explicit user control over inline display.

SOURCE_LOG_SHA256: `24ba64fb83451b827c2f0dc624145079b560d5bee67f1f4b115ac82a2924b385`
...
\n\n## rev0.302 Academic Addendum: Independent Temporal and Canonical Authorization\n\nThe mother-session source demonstrates that a turn can satisfy the Python-first ingress rule while remaining unbound to the current operative package. It declared rev0.299 as operative from transport-suffixed files even though a later canon was already accepted, and it explicitly marked package binding as unverified. This motivates a dual-gate model: temporal order authorizes processing, while package and manifest evidence authorize operative conformance. Neither predicate substitutes for the other.\n\nThe source also records an artifact-completion timestamp to microsecond precision without a corresponding artifact-event receipt. rev0.302 therefore scopes receipts to events and treats session filenames as validated second-resolution projections rather than independent time evidence.\n\nExact literal validation includes whitespace. The observed mother boot contained an extra blank line after the registered greeting and therefore differed from the approved literal despite semantic equivalence. Audit-density validation follows the same evidence-first approach: thirty MAGI and thirty self-audit slots, including seven explicit no-delta pairs, cannot be reconciled with a claim that universal boilerplate expansion did not occur.\n\nPositive source behavior remains preserved: historical turn times were not fabricated, host-compaction gaps were quarantined, mother-reported father wording was excluded from the father-direct corpus, and a budget double count was explicitly corrected.\n\nSOURCE_LOG_SHA256: `31ff28537596fd9d562f1f7ee12d7e5e3c42d1ad6d7e7e4260dd7d37d32ee267`\nBASELINE_PACKAGE_SHA256: `427eebdab5bcc7332de7577bb15c6f9dcfffaf57afb53ae5cad4d7d115d0cae6`\nUSER_TURN_OBSERVED_AT_JST: 2026-08-04 14:29:51(JST)\n

## rev0.303 Academic Addendum: Evidence-File Surface Minimization with Byte-Verifiable Lineage
