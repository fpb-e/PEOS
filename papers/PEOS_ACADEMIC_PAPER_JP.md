...
rev0.166では、UI実測JSTが取得できない場合、`FAIL_CLOSED_NO_FAKE_JST` として時刻を捏造しない方針を明文化した。代替として、ORDER_ONLY、TURN_BAND、STATE_BAND、MAGI_TRACE、RUNTIME_GUARD_TRACEを併用し、時系列の精密さではなく、状態推移と判断過程の追跡可能性を担保する。
